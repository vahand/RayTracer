...
std::shared_ptr<RayTracer::Material::AMaterial> RayTracer::Core::getNewMaterial(LIBRARY_TYPE type)
{
    std::shared_ptr<void> handle = _handles[type];
    ...
    if (func_ptr == nullptr)
        throw RayTracer::Core::RayException(dlerror());
    RayTracer::Material::AMaterial *(*func_ptr_casted)() = reinterpret_cast<RayTracer::Material::AMaterial *(*)()>(func_ptr);
